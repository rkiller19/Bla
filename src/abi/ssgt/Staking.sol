...
contract NFT is ERC721URIStorage, Ownable{
    ...
    function removeTokenIdFromUser(uint256 _tokenId) public {
        require(msg.sender == minter, "You can't mint a token");
        for(uint i = 0; i < userToken[ownerOf(_tokenId)].tokenId.length - 1; i++){
    ...
    function burnInBulk(uint256[] memory _tokenId) public{
        for(uint i = 0; i < _tokenId.length - 1; i++){
...
contract Staking is Ownable, IERC721Receiver{
    ...
    constructor(address _tokenAddress, address _nftContractAddress, uint _rewardRate, uint _rewardInterval, uint _stakingFeeRate, uint _unstakingFeeRate, uint _cliffTime, uint _tokenAmount) {
        nftToken = NFT(_nftContractAddress);
    ...
    function NFTTokenAddress() public view returns(NFT){
        return nftToken;
    }
    ...
    /**
     * @notice Change Token Amount
     */
    function changeTokenAmount(uint _tokenAmount) public onlyOwner{
        tokenAmount = _tokenAmount;
    }
     ...
     */
    function mintNFTToken(uint amountToStake) public {
        uint fee = amountToStake.mul(stakingFeeRate).div(1e4);
        uint amountAfterFee = amountToStake.sub(fee);
        totalStakedToken = totalStakedToken.add(amountAfterFee);
        if(amountAfterFee >= tokenAmount){
            uint amountOfToken = amountAfterFee.div(tokenAmount);
            for(uint i = 0; i < amountOfToken; i++){
                nftToken.mintToken(msg.sender, "uri goes here");
            }
            uint remainingToken = amountAfterFee.sub(amountOfToken.mul(tokenAmount));
            depositedTokens[msg.sender] = depositedTokens[msg.sender].add(remainingToken);
    ...
    function burnNFTOfContract(uint _tokenId) public {
        nftToken.burnNFT(_tokenId);
    }
